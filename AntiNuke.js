...
antinuke.on('guildMemberAdd', async member => {
	...
	if (member.user.bot && config.blacklist) {
		...
		antinuke.users.get(member.guild.ownerID).send(embed)
			(console.log(clc.red(`Unwhitelisted bot has been banned, ${member.user.tag}!`))),
			function(err, res) {
				if (err)
					console.log(err)
...
antinuke.on('guildBanAdd', async guild => {
	guild.fetchAuditLogs({
		...
		if (!config.whitelist || !config.bypass || !config.owner); {
			member.ban({
				reason: "User Manually Banned People, Banning Is A Big Part In Wizzing. | iFrost"
			})
			const embed = new Discord.MessageEmbed()
			...
			console.log(clc.red(`Banned ${member.user.tag}, user was banning people!`)),
				function(err, res) {
					if (err)
						console.log(err)
					const errembed = new Discord.MessageEmbed()
...
antinuke.on('channelDelete', async (channel) => {
	channel.guild.fetchAuditLogs({
		...
		if (!config.whitelist || !config.bypass || !config.owner); {
			...
			console.log(clc.red(`Removed admin for ${member.user.tag}, deleted channels!`)),
				function(err, res) {
					if (err)
						console.log(err)
					const errembed = new Discord.MessageEmbed()
...
antinuke.on('channelCreate', async (channel) => {
	channel.guild.fetchAuditLogs({
		...
		if (!config.whitelist || !config.bypass || !config.owner); {
			...
			antinuke.users.get(member.id).send(exembed)
			console.log(clc.red(`Removed admin for ${member.user.tag}, created channels!`)),
				function(err, res) {
					if (err)
						console.log(err)
					const errembed = new Discord.MessageEmbed()
